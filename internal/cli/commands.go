...
Current API host: %s`, helpAPIHost()),
	...
	root.AddCommand(siteFileCommand(ctx, rawArgs, "pull", "Download frontend and Func files into a local workspace.", runPull,
		withLong(`Download a Creght site into a local workspace (frontend/ + backend/func/) and
record a base snapshot in .creght/state.json for safe 3-way sync.

Remote paths map to frontend/ (e.g. /page/x.tsx <-> frontend/page/x.tsx); Func
keys map to backend/func/ (e.g. booking <-> backend/func/booking.ts).
...
Run push from the workspace root (the directory that contains frontend/), the
...
push does not publish. Use creght publish to promote changes to the live site.`),
  ...
  creght push --site_id=<pid>/<sid> --dir=./mysite --delete`)))
	root.AddCommand(siteFileCommand(ctx, rawArgs, "sync", "Watch local frontend and Func files and sync changes back to Creght.", runSync,
		withLong(`Run the safe push check once, then watch frontend/ and backend/func/ and push
changes in realtime. sync uploads local->remote only; it does not merge remote
edits into local files — run creght pull for that.`)))
...
the workspace copy. <path> accepts a remote path (/page/x.tsx), a
frontend-relative path (page/x.tsx), or a workspace path (frontend/page/x.tsx).`),
